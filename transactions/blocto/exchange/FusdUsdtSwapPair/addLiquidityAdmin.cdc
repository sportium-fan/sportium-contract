...
transaction(token1Amount: UFix64, token2Amount: UFix64) {
  ...
  let admin: &FusdUsdtSwapPair.Admin
  ...
  prepare(signer: AuthAccount) {
    ...
    self.admin = signer.borrow<&FusdUsdtSwapPair.Admin>(from: /storage/fusdUsdtPairAdmin)
  ...
  execute {
    ...
    let liquidityTokenVault <- FusdUsdtSwapPair.addInitialLiquidity(from: <- tokenBundle)
